...
| 文档 | 读者 | 内容 |
|---|---|---|
| **本文档** | 所有人 | 项目简介、快速开始、功能清单、常见问题 |
| [docs/USAGE.md](docs/USAGE.md) | 普通用户 | **完整使用手册**：面板详解、积分策略怎么选、账号标识含义、疑难排查 |
| [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) | 开发者 / AI Agent | **架构与实现**：代码地图、关键不变量、数据流、前后端契约、构建发布、跨平台移植 |
| [docs/trae-integration.md](docs/trae-integration.md) | 开发者 | TraeWork 上游协议细节 |
| [docs/lesson-dual-process.md](docs/lesson-dual-process.md) | 开发者 | 双进程架构踩坑记录 |

## 更新记录

### v0.4.1（2026-09-23）
- **新增总积分显示**：账号列表上方常驻显示所有账号的积分总和，实时更新（数值增加时有一次绿色跳动提示）
- 总积分右侧附带账号可用情况（如 `3/5 个可用`），有账号处于冷却或禁用时一眼可见

### v0.4.0（2026-09-22）
- **账号调用标识**：账号卡片实时显示「调用中」（绿色脉冲徽标）与「最近用过」，一眼看出当前在用哪个账号
- **积分策略可选**：面板新增策略下拉，支持**优先积分**（默认，用积分最多的）/ **优先过期**（快过期的凭证先用，避免浪费）/ **负载均衡**（轮流使用每个账号），切换即写回 config.json 全局生效
  - 配置文件对应字段：`"strategy": "credits" | "expire" | "roundrobin"`
  - 「优先积分」策略下积分相同时按账号稳定排序（修复原先同分选中不确定的问题）
- **面板布局重做**：窗口加宽至 760×560 固定尺寸，**左侧账号列表 / 右侧设置栏**双栏显示；账号再多也不用滚整页，列表在左栏内部滚动
- 默认**浅色**外观（不再跟随系统深色，避免深色系统下看不清），右上角 🌙/☀ 仍可手动切换
- 账号列表排序优化：正在调用的账号自动置顶
- 新增 `max_rotate` 配置项（单个请求最多轮换的账号数，默认 3）
...
- 本项目把多个账号聚合成一个 **OpenAI 兼容 API**，按可选的策略（优先积分 / 优先过期 / 负载均衡）挑选账号、自动刷新 token、错误自动冷却
...
- ✅ **三种选号策略**：优先积分（默认）/ 优先过期 / 负载均衡，面板下拉切换即生效
- ✅ **账号调用标识**：实时显示「调用中」/「最近用过」，正在调用的账号自动置顶
- ✅ **双栏面板**：左账号列表（内部滚动）/ 右设置栏，账号再多也不挤
- ✅ 托盘面板：添加账号（无痕浏览器）、自动签到时间配置、签到状态、积分实时刷新
- ✅ API 监听主机/端口可改（`127.0.0.1` 本机 / `0.0.0.0` 全部 / 自定义），热切换
- ✅ 账号凭证 DPAPI 加密落盘、日志自动轮转、开机自启、面板位置记忆
